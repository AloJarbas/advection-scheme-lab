...
def write_limiter_family_followup_csv(output: Path) -> None:
    rows = study_transport(
        schemes=("lax-wendroff", "tvd-minmod", "tvd-mc", "tvd-superbee"),
        requested_cfls=LIMITER_FAMILY_CFLS,
    )
    output.parent.mkdir(parents=True, exist_ok=True)
    with output.open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(rows[0].as_dict().keys()), lineterminator="\n")
...
def render_limiter_family_followup(svg_output: Path, png_output: Path | None = None, *, cfl: float = 0.95) -> None:
    ...
    svg = render_limiter_family_followup_svg(gaussian_runs, square_runs, rows)
    write_svg(svg, svg_output)
    if png_output is not None:
        export_png(svg_output, png_output)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    render_limiter_family_parser = subparsers.add_parser(
        "render-limiter-family-followup",
        help="render the limiter-family follow-up SVG and optional PNG",
    )
    render_limiter_family_parser.add_argument("--output", required=True)
    render_limiter_family_parser.add_argument("--png-output")
    render_limiter_family_parser.add_argument("--cfl", type=float, default=0.95)

    limiter_family_csv_parser = subparsers.add_parser(
        "write-limiter-family-csv",
        help="write the limiter-family follow-up CSV",
    )
...
def main() -> None:
    ...
    if args.command == "write-cfl-sweep-csv":
        ...
        return
    if args.command == "render-limiter-family-followup":
        render_limiter_family_followup(Path(args.output), Path(args.png_output) if args.png_output else None, cfl=args.cfl)
        return
